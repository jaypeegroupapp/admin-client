...
import { confirmSupplierInvoicePaymentAction } from "@/actions/supplier-invoice";
import { Check } from "lucide-react";

export function ConfirmSupplierInvoicePaymentModal({
  invoiceId,
  open,
  onClose,
}: {
  ...
  const handlePayment = () => {
    startTransition(async () => {
      const result = await confirmSupplierInvoicePaymentAction(invoiceId);
  ...
  return (
    <BaseModal open={open} onClose={onClose}>
      ...
      <p className="text-sm text-gray-600">
        Confirm that this supplier invoice has been paid?
      </p>
