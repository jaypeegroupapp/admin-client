...
import { Search, User, Gauge } from "lucide-react";
...
export function AssignAttendantModal({
  ...
  const {
    ...
    defaultValues: {
      openingBalance: currentMeterReading,
    },
  });
        ...
        <form ref={formRef} onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            <p className="text-sm text-blue-600 font-medium flex items-center gap-2">
              <Gauge size={14} /> Current Meter Reading
            </p>
            <p className="text-2xl font-bold text-blue-700">
              {currentMeterReading.toLocaleString()}L
            </p>
            <p className="text-xs text-blue-600 mt-1">
              This will be recorded as opening meter reading for the shift
            </p>
          </div>
          ...
          <SubmitButton name="Start Shift" isPending={isPending} />
        </form>
      </div>
    </BaseModal>
  );
